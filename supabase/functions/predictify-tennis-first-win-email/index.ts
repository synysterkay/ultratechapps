// Supabase Edge Function: predictify-tennis-first-win-email
//
// Fires INSTANTLY when a Predictify Tennis user's first prediction resolves
// correct — the emotional peak that turns a trial user into a habit user.
//
//   POST .../functions/v1/predictify-tennis-first-win-email
...
const TEMPLATES: Record<string, Template> = {
  en: {
    ...
    body: [
      "Hey {{first_name}},",
      "{{home_team}} vs {{away_team}} — you picked it, the model picked it, and the match agreed. That's your first correct tennis prediction on the board.",
  ...
  es: {
    ...
    body: [
      "Hola {{first_name}},",
      "{{home_team}} vs {{away_team}} — tú lo predijiste, el modelo lo predijo y el partido lo confirmó. Esa es tu primera predicción de tenis acertada.",
  ...
  fr: {
    ...
    body: [
      "Bonjour {{first_name}},",
      "{{home_team}} vs {{away_team}} — vous l'avez pronostiqué, le modèle aussi, et le match a confirmé. C'est votre premier pronostic tennis réussi.",
  ...
  ar: {
    ...
    body: [
      "مرحبًا {{first_name}}،",
      "{{home_team}} ضد {{away_team}} — أنت توقعته، والنموذج توقعه، والمباراة أكدت ذلك. هذا أول توقع تنس صحيح لك.",
  ...
  pt: {
    ...
    body: [
      "Olá {{first_name}},",
      "{{home_team}} vs {{away_team}} — você previu, o modelo previu, e o jogo confirmou. Essa é a sua primeira previsão certa de tênis.",
  ...
  de: {
    ...
    body: [
      "Hallo {{first_name}},",
      "{{home_team}} vs {{away_team}} — du hast es getippt, das Modell auch, und das Spiel hat es bestätigt. Das ist deine erste richtige Tennis-Vorhersage.",
  ...
  tr: {
    ...
    body: [
      "Merhaba {{first_name}},",
      "{{home_team}} - {{away_team}} — sen tahmin ettin, model tahmin etti ve maç doğruladı. Bu senin ilk doğru tenis tahminin.",
  ...
  it: {
    ...
    body: [
      "Ciao {{first_name}},",
      "{{home_team}} vs {{away_team}} — l'hai pronosticato tu, l'ha fatto il modello, e la partita ha confermato. È il tuo primo pronostico tennis azzeccato.",
  ...
  nl: {
    ...
    body: [
      "Hoi {{first_name}},",
      "{{home_team}} vs {{away_team}} — jij koos het, het model koos het, en de wedstrijd bevestigde het. Dat is je eerste juiste tennisvoorspelling.",
  ...
  pl: {
    ...
    body: [
      "Cześć {{first_name}},",
      "{{home_team}} vs {{away_team}} — Ty to wytypowałeś, model też, a mecz potwierdził. To Twoja pierwsza trafiona prognoza tenisowa.",
  ...
  ja: {
    ...
    body: [
      "{{first_name}}さん、こんにちは。",
      "{{home_team}} 対 {{away_team}} — あなたが選び、モデルも選び、試合がそれを証明しました。これがあなたの初めての的中テニス予想です。",
  ...
  id: {
    ...
    body: [
      "Hai {{first_name}},",
      "{{home_team}} vs {{away_team}} — kamu memilihnya, model memilihnya, dan pertandingan membuktikannya. Itu prediksi tenis pertamamu yang benar.",
...
const FOOTERS: Record<string, string> = {
  en: "You're receiving this because your first prediction on Predictify Tennis just resolved correct.",
  es: "Recibes esto porque tu primera predicción en Predictify Tennis acaba de acertar.",
  fr: "Vous recevez ceci car votre premier pronostic sur Predictify Tennis vient d'être validé.",
  ar: "تتلقى هذا لأن أول توقع لك على Predictify Tennis كان صحيحًا للتو.",
  pt: "Você recebe isto porque sua primeira previsão no Predictify Tennis acabou de acertar.",
  de: "Du erhältst dies, weil deine erste Vorhersage bei Predictify Tennis gerade richtig war.",
  tr: "Bunu, Predictify Tennis'deki ilk tahmininiz az önce doğru çıktığı için alıyorsunuz.",
  it: "Ricevi questo perché il tuo primo pronostico su Predictify Tennis è appena risultato corretto.",
  nl: "Je ontvangt dit omdat je eerste voorspelling op Predictify Tennis zojuist juist bleek.",
  pl: "Otrzymujesz to, bo Twoja pierwsza prognoza w Predictify Tennis właśnie się sprawdziła.",
  ja: "Predictify Tennisでの初めての予想が的中したため、このメールをお送りしています。",
  hi: "आपको यह इसलिए मिल रहा है क्योंकि Predictify Tennis पर आपकी पहली भविष्यवाणी अभी सही साबित हुई।",
  id: "Kamu menerima ini karena prediksi pertamamu di Predictify Tennis baru saja terbukti benar.",
};
