// Shared core for Predictify Tennis instant emails.
//
// Three thin entrypoints (predictify-tennis-first-win-email,
// predictify-tennis-paywall-hit-email, predictify-tennis-streak-broken-email,
// predictify-tennis-leaderboard-email)
// import handleTennisEmail() and pass their own multilingual template table.
...
// tiny and consistent.
//
// App scope: app_slug = "predictify_tennis". Suppressions and dedup are keyed
// on this slug so Tennis and soccer Predictify never cross-contaminate.

import { createClient } from "jsr:@supabase/supabase-js@2";
import { SENDER_POOL_TENNIS as SENDER_POOL, sanitizeSubject } from "./sender_pool.ts";
...
export const APP_NAME = "Predictify Tennis";
export const APP_SLUG = "predictify_tennis";
const DEEPLINK_BASE = "https://tenis-b5d4e.web.app";
...
function buildHtml(
  ...
  const body = paragraphs
    ...
    .join("");
  // Court-green gradient on the CTA (matches the tennis app primary).
...
<a href="${ctaHref}" style="display:inline-block;background:linear-gradient(135deg,#2EC98A 0%,#1B9E6E 100%);color:#fff;padding:14px 32px;text-decoration:none;border-radius:8px;font-weight:700;font-size:16px;">🎾 ${ctaText}</a>
</div>
...
async function buildUnsubUrl(email: string): Promise<string> {
  ...
  // suppression to predictify_tennis, not soccer predictify.
...
export async function handleTennisEmail(req: Request, cfg: EmailConfig): Promise<Response> {
  ...
  const vars = {
    ...
    league_name: String(payload.league_name || "ATP"),
