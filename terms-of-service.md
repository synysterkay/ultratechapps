---
layout: default
title: Terms of Service
permalink: /terms-of-service/
---

<div class="legal-container">
  <h1 class="legal-title">Terms of Service</h1>
  <p class="legal-updated">Last Updated: January 8, 2026</p>

  <div class="legal-content">
    <p>Welcome to UltraTech Apps. By accessing our website or using our mobile applications, you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, please do not use our services.</p>

    <h2>1. Acceptance of Terms</h2>
    <p>By downloading, installing, or using any UltraTech Apps application, you acknowledge that you have read, understood, and agree to be bound by these Terms and our <a href="/marketingtool/privacy-policy/">Privacy Policy</a>.</p>

    <h2>2. Eligibility</h2>
    <p>You must be at least 13 years old (or 16 in the EU) to use our services. By using our apps, you represent and warrant that you meet this age requirement and have the legal capacity to enter into these Terms.</p>

    <h2>3. License to Use</h2>
    <p>Subject to your compliance with these Terms, we grant you a limited, non-exclusive, non-transferable, revocable license to:</p>
    <ul>
      <li>Download and install our apps on your personal devices</li>
      <li>Use the apps for personal, non-commercial purposes</li>
      <li>Access features available in your chosen subscription tier</li>
    </ul>
    <p>This license does not include the right to:</p>
    <ul>
      <li>Modify, reverse engineer, or decompile our apps</li>
      <li>Remove copyright or proprietary notices</li>
      <li>Use our apps for commercial purposes without authorization</li>
      <li>Create derivative works or competitive products</li>
    </ul>

    <h2>4. User Accounts</h2>
    <p>Some apps may require you to create an account. You are responsible for:</p>
    <ul>
      <li>Maintaining the confidentiality of your account credentials</li>
      <li>All activities that occur under your account</li>
      <li>Notifying us immediately of any unauthorized access</li>
      <li>Providing accurate and current information</li>
    </ul>
    <p>We reserve the right to suspend or terminate accounts that violate these Terms.</p>

    <h2>5. Subscriptions and Payments</h2>
    
    <h3>5.1 Free and Premium Features</h3>
    <p>Our apps may offer both free features and premium subscriptions. Premium features require payment through the App Store or Google Play.</p>

    <h3>5.2 Billing</h3>
    <ul>
      <li>Subscriptions auto-renew unless canceled 24 hours before the renewal date</li>
      <li>Prices are displayed in your local currency at the time of purchase</li>
      <li>Payment is charged through your App Store or Google Play account</li>
      <li>We do not store or process payment information directly</li>
    </ul>

    <h3>5.3 Refunds</h3>
    <p>Refund requests are handled by Apple App Store or Google Play according to their policies. We do not control or process refunds directly. Contact the respective store for refund inquiries.</p>

    <h3>5.4 Cancellation</h3>
    <p>You can cancel your subscription at any time through your App Store or Google Play account settings. Cancellation takes effect at the end of your current billing period.</p>

    <h2>6. User Content</h2>
    
    <h3>6.1 Your Content</h3>
    <p>You retain ownership of content you create using our apps (notes, recordings, etc.). By using our services, you grant us a limited license to store, process, and display your content to provide app functionality.</p>

    <h3>6.2 Prohibited Content</h3>
    <p>You may not use our apps to create, store, or share content that:</p>
    <ul>
      <li>Violates laws or regulations</li>
      <li>Infringes intellectual property rights</li>
      <li>Contains malware or harmful code</li>
      <li>Is harassing, abusive, or offensive</li>
      <li>Promotes illegal activities</li>
    </ul>

    <h2>7. Acceptable Use</h2>
    <p>You agree NOT to:</p>
    <ul>
      <li>Use our apps for any illegal purpose</li>
      <li>Attempt to hack, breach, or circumvent security measures</li>
      <li>Interfere with other users' access to services</li>
      <li>Scrape, data-mine, or automate access without permission</li>
      <li>Impersonate others or misrepresent your affiliation</li>
      <li>Upload viruses or malicious code</li>
      <li>Abuse or exploit our support services</li>
    </ul>

    <h2>8. Intellectual Property</h2>
    <p>All rights, title, and interest in our apps, including:</p>
    <ul>
      <li>Source code, algorithms, and architecture</li>
      <li>User interface designs and graphics</li>
      <li>Trademarks, logos, and brand assets</li>
      <li>Documentation and content</li>
    </ul>
    <p>...are owned by UltraTech Apps or our licensors and protected by copyright, trademark, and other intellectual property laws.</p>

    <h2>9. Third-Party Services</h2>
    <p>Our apps may integrate with or link to third-party services (AI APIs, analytics, social media). We are not responsible for:</p>
    <ul>
      <li>Third-party content, products, or services</li>
      <li>Privacy practices of third parties</li>
      <li>Availability or performance of external services</li>
    </ul>
    <p>Your use of third-party services is governed by their respective terms and policies.</p>

    <h2>10. Disclaimers and Limitations</h2>
    
    <h3>10.1 "AS IS" Basis</h3>
    <p>Our apps are provided "AS IS" and "AS AVAILABLE" without warranties of any kind, express or implied, including but not limited to:</p>
    <ul>
      <li>Merchantability or fitness for a particular purpose</li>
      <li>Accuracy, reliability, or completeness of content</li>
      <li>Uninterrupted or error-free operation</li>
      <li>Security or absence of viruses</li>
    </ul>

    <h3>10.2 Limitation of Liability</h3>
    <p>TO THE MAXIMUM EXTENT PERMITTED BY LAW, UltraTech Apps SHALL NOT BE LIABLE FOR:</p>
    <ul>
      <li>Indirect, incidental, special, or consequential damages</li>
      <li>Loss of profits, data, or business opportunities</li>
      <li>Damages resulting from unauthorized access or data breaches</li>
      <li>Issues caused by third-party services or integrations</li>
    </ul>
    <p>Our total liability shall not exceed the amount you paid for the app in the past 12 months, or $100 USD, whichever is less.</p>

    <h2>11. Indemnification</h2>
    <p>You agree to indemnify and hold harmless UltraTech Apps, its developers, and affiliates from any claims, damages, or expenses arising from:</p>
    <ul>
      <li>Your violation of these Terms</li>
      <li>Your use or misuse of our apps</li>
      <li>Your violation of any laws or third-party rights</li>
      <li>Content you create or share using our services</li>
    </ul>

    <h2>12. Termination</h2>
    <p>We reserve the right to suspend or terminate your access to our apps at any time, with or without notice, for:</p>
    <ul>
      <li>Violation of these Terms</li>
      <li>Fraudulent or illegal activity</li>
      <li>Abuse of our services or support</li>
      <li>Non-payment of subscription fees</li>
    </ul>
    <p>Upon termination, your license to use the app ends immediately, and you must delete all copies from your devices.</p>

    <h2>13. Changes to Terms</h2>
    <p>We may modify these Terms at any time. Changes will be effective immediately upon posting. Your continued use of our apps after changes constitutes acceptance. We recommend reviewing these Terms periodically.</p>

    <h2>14. Governing Law</h2>
    <p>These Terms are governed by the laws of [Your Jurisdiction], without regard to conflict of law principles. Any disputes shall be resolved in the courts of [Your Jurisdiction].</p>

    <h2>15. Dispute Resolution</h2>
    <p>For disputes, you agree to first contact us to seek informal resolution. If unresolved within 30 days, disputes may be submitted to binding arbitration under [Arbitration Rules], excluding class actions.</p>

    <h2>16. Severability</h2>
    <p>If any provision of these Terms is found invalid or unenforceable, the remaining provisions shall remain in full force and effect.</p>

    <h2>17. Contact Information</h2>
    <p>For questions about these Terms, contact us at:</p>
    <ul>
      <li><strong>Email:</strong> legal@ultratechapps.com</li>
      <li><strong>Website:</strong> <a href="https://synysterkay.github.io/marketingtool/">UltraTech Apps</a></li>
      <li><strong>Developer:</strong> Anas Kayssi</li>
    </ul>

    <div class="legal-notice">
      <p><strong>Acknowledgment:</strong> By using our apps, you acknowledge that you have read these Terms of Service and agree to be bound by them.</p>
    </div>
  </div>
</div>
