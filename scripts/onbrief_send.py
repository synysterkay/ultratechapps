#!/usr/bin/env python3
"""Shared send + daily cap for Onbrief retention emails."""
import hashlib
import json
import os
...
from onbrief_email_chrome import render as render_email
...
DAILY_CAP = int(os.getenv('ONBRIEF_DAILY_SEND_CAP', '30'))
_REF_SALT = os.getenv('EMAIL_REF_SALT', 'marketing-tool-v1')
QUOTA_FILE = Path(__file__).parent.parent / 'cache' / 'onbrief_daily_quota.json'
...
def send_onbrief(sender, *, email, subject, paragraphs, cta, kind, lang='en',
                 stage=None, gradient='invite', from_name=None, signoff_override=None,
                 cta_url=None):
    if remaining() <= 0:
        print('   ⏭️ Onbrief daily cap reached')
        return 'throttled'
