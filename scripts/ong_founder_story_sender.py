#!/usr/bin/env python3
"""Founder story — ≥14d inactive. Lapsed catch-up, once. From ONG / Alex."""
import sys
from pathlib import Path
from datetime import datetime

sys.path.insert(0, str(Path(__file__).parent))

from ong_users_loader import get_access_token, load_all_users, days_since_open
from ong_send import connect_sender, send_ong, load_state, save_state, remaining, APP_SLUG
from ong_templates import get_template, fill

KIND = f'{APP_SLUG}_founder_story'
STATE_FILE = Path(__file__).parent.parent / 'cache' / 'ong_founder_story_state.json'
LAPSED_DAYS = 14


def main(dry_run=False):
    token = get_access_token()
    if not token:
        print('⚠️ FIREBASE_TOKEN not set')
        return

    state = load_state(STATE_FILE)
    sent_uids = set((state.get('users') or {}).keys())
    candidates = []
    for user in load_all_users(token):
        if user['uid'] in sent_uids:
            continue
        days = days_since_open(user)
        if days < LAPSED_DAYS:
            continue
        candidates.append((user, days))

    print(f'✉️  {len(candidates)} founder-story candidates (≥{LAPSED_DAYS}d)')
    if dry_run:
        for u, d in candidates[:15]:
            print(f"   • {u['email']}  ({d}d)")
        return
    if not candidates:
        return

    sender = connect_sender()
    if not sender:
        return
    tpl = get_template('founder_story')
    sent = failed = 0
    for user, days in candidates:
        if remaining() <= 0:
            break
        filled = fill(tpl, first_name=user.get('first_name', 'there'))
        result = send_ong(
            sender, email=user['email'], subject=filled['subject'],
            paragraphs=filled['body'], cta=filled['cta'], kind=KIND,
            gradient='calm', from_name='Alex',
        )
        if result == 'sent':
            sent += 1
            state['users'][user['uid']] = {
                'sent_at': datetime.now().isoformat(),
                'inactive_days': days,
            }
            print(f"   ✅ [{sent}] {user['email']}  ({days}d)")
        else:
            failed += 1
    sender.disconnect()
    save_state(STATE_FILE, state)
    print(f'📊 Done — sent {sent}, failed {failed}')


if __name__ == '__main__':
    main(dry_run='--dry-run' in sys.argv)
