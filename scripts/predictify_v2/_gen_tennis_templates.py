#!/usr/bin/env python3
"""One-shot author of the 14 Tennis Hooked-model English templates.

Mirrors the soccer/NBA templates' kinds + merge fields EXACTLY (so triggers.py
and template_engine.py work unchanged), but the copy is tennis-tuned: first
serve, Elo/surface/form, match winner / total games / games handicap, ATP/WTA
and slams. Writes to templates_tennis/{kind}_en.json.
Run once: python3 scripts/predictify_v2/_gen_tennis_templates.py
"""
import json
import os

OUT = os.path.join(os.path.dirname(__file__), 'templates_tennis')
os.makedirs(OUT, exist_ok=True)

T = {
    "welcome": {
        "subject": "Your first tennis call starts here, {first_name}",
        "preview_text": "{top_match_line}",
        "body_paragraphs": [
            "Hey {first_name},",
            "Welcome to Predictify. Quick orientation: every ATP and WTA match in the app comes with a calibrated AI confidence score — built from Elo, surface, recent form, and head-to-head — that tells you exactly how reliable the call is. Numbers, not hot takes.",
            "Try it now: {top_match_line}. Tap the match, see the full breakdown — winner, total games, games handicap — and lock in your call.",
            "Pro tip: make one pick a day for 5 days and you build a streak. Streak users are 3x more likely to spot the value calls before first serve.",
        ],
    ...
    "match_day": {
        ...
        "preview_text": "{league_name} starts in {hours_to_kickoff}h. Your AI's pick is ready.",
        "body_paragraphs": [
            "Hey {first_name},",
            "{home_team} vs {away_team} walks on court in about {hours_to_kickoff} hours. Your AI has crunched every signal — Elo, surface, recent form, H2H, and serve stats — and locked in a call.",
            "\U0001F4CA The pick: {pick_label}\n\U0001F3AF Calibrated confidence: {confidence_pct}%\n\U0001F3BE Tier: {tier_label}",
            "This is exactly what we built Predictify for. Open the app, see the full reasoning, and lock in your prediction before first serve.",
        ],
    ...
    "login_streak_reward": {
        ...
        "body_paragraphs": [
            ...
            "Your reward: 7 days of Predictify Pro, fully unlocked, no payment needed. You'll see the Strongest Signal on every match, calibrated confidence on every market (winner, total games, handicap, first set), and the full prediction history.",
    ...
    "pro_power_tip": {
        ...
        "preview_text": "It hits hardest on Elite-tier matches. Most users never open it.",
        "body_paragraphs": [
            "Hey {first_name},",
            "Quick Pro tip you may have missed: on every prediction screen, scroll past the set-score projection and find the 'Strongest Signal' card. It's the highest-confidence, calibrated pick for that match — and on Elite-tier matchups it's historically been the safest call on the board.",
            "How to use it: when the tier badge says Elite or Premium, the Strongest Signal is your anchor. When it says Standard, treat it as a starting point and weigh the other markets too.",
    ...
    "pro_owner_pitch": {
        ...
        "body_paragraphs": [
            "Hey {first_name},",
            "You've been Pro for a while and your picks are consistent. Time to flip the model: open your own tennis community on Predictify, set a small monthly price, and keep 70% of every subscription.",
            ...
            "Setup is 90 seconds: pick a name, pick your angle (a tour, a player, a betting style), set a price or keep it free. Owners with a cover image grow 3x faster — we generate one for you in a tap.",
        ],
    ...
    "owner_marketing_kit": {
        ...
        "body_paragraphs": [
            ...
            "1. WhatsApp/Telegram group chats: drop the link with a one-line tease (\"AI calls for every ATP match, free to join\")\n2. X / Twitter: post your most confident pick of the day with the join link in the reply\n3. r/tennis, r/sportsbook on Reddit: a pinned daily thread\n4. TikTok comments under tennis preview / parlay videos\n5. Discord servers for the tours you cover",
            "Predictify makes the cover image for you (your share link looks 3x better with one — that alone moves join rate). Open the share kit, copy the link, paste it where tennis fans gather.",
        ],
    ...
    "owner_growth": {
        ...
        "body_paragraphs": [
            ...
            "We've made the share image for you. One tap and it's ready for WhatsApp, Discord, or wherever your tennis people hang out.",
        ],
        "cta_text": "Open share kit",
        "cta_deeplink": "predictify://community/{community_id}/share",
    },
    "referral_invite": {
        "subject": "Get free Pro by inviting one tennis friend",
        "preview_text": "1 referral = 1 month free for both of you.",
        "body_paragraphs": [
            "Hey {first_name},",
            "You've been making picks consistently — the part most users skip. Now the easy bonus: invite one friend who watches the same matches you do, and you both get a free month of Predictify Pro the moment they sign up.",
    ...
    "community_invite": {
        ...
        "body_paragraphs": [
            "Hey {first_name},",
            "Picking matches alone is fine. Picking with people who follow the same tours? Way better.",
...
for kind, data in T.items():
    ...
    with open(path, "w") as f:
        ...
        f.write("\n")
    count += 1
print(f"Wrote {count} Tennis English templates to {OUT}")
