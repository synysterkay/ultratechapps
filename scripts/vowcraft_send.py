#!/usr/bin/env python3
"""Shared send + daily cap for Vowcraft retention emails."""
import hashlib
import json
import os
...
from vowcraft_email_chrome import render as render_email

APP_NAME = 'Vowcraft'
APP_SLUG = 'vowcraft'
DEEP_LINK = os.getenv(
    'VOWCRAFT_CTA_URL',
    'https://play.google.com/store/apps/details?id=com.vowcraft.wedding.speech',
)
DAILY_CAP = int(os.getenv('VOWCRAFT_DAILY_SEND_CAP', '30'))
_REF_SALT = os.getenv('EMAIL_REF_SALT', 'marketing-tool-v1')
QUOTA_FILE = Path(__file__).parent.parent / 'cache' / 'vowcraft_daily_quota.json'
...
def send_vowcraft(sender, *, email, subject, paragraphs, cta, kind, lang='en',
                  stage=None, gradient='invite', from_name=None, signoff_override=None,
                  cta_url=None):
    if remaining() <= 0:
        print('   ⏭️ Vowcraft daily cap reached')
        return 'throttled'
    sender_name = from_name or APP_NAME
    html = render_email(
        lang, paragraphs, cta, cta_url or DEEP_LINK,
