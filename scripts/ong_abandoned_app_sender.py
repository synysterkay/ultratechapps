...
from ong_users_loader import get_access_token, load_all_users, is_paid, days_since_open
...
KIND = f'{APP_SLUG}_abandoned_app'
STATE_FILE = Path(__file__).parent.parent / 'cache' / 'ong_abandoned_app_state.json'
STAGES = [10, 5, 2]
...
def main(dry_run=False):
    ...
    for user, stage, days in candidates:
        if remaining() <= 0:
            break
        tpl = fill(get_template('abandoned_app', stage), first_name=user.get('first_name', 'there'))
        result = send_ong(
